  ...
  return (
    <div className="flex items-center gap-0">
      <button
        ...
        disabled={value <= min}
        className="w-8 h-8 border border-brand-mid/50 text-brand-cream/70 hover:border-brand-gold hover:text-brand-gold disabled:opacity-30 disabled:cursor-not-allowed transition-colors flex items-center justify-center font-sans text-base"
      >−</button>
      <div className="w-10 h-8 border-t border-b border-brand-mid/50 flex items-center justify-center">
        <span className="font-sans text-brand-cream font-medium text-sm">{value}</span>
      </div>
      <button
        type="button"
...
export default function Order() {
  ...
  const totalPacks = FLAVORS.reduce((sum, f) => sum + (quantities[f.value] ?? 0), 0);
  const subtotal = FLAVORS.reduce((sum, f) => sum + (quantities[f.value] ?? 0) * f.price, 0);
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col gap-3">
              <label className="font-sans tracking-widest text-xs text-brand-cream/50 uppercase">Select Flavours</label>
              {FLAVORS.map(f => (
                <div key={f.value} className={`flex items-center justify-between py-2.5 px-3 border ${f.available ? 'border-brand-mid/50' : 'border-brand-mid/20 opacity-40'}`}>
                  <div className="flex flex-col">
                    <span className={`font-sans text-sm ${f.available ? 'text-brand-cream' : 'text-brand-cream/40'}`}>
                      {f.label}
                    </span>
                    <span className="font-sans text-xs text-brand-cream/40 tracking-wider">
                      {f.available ? `₦${f.price.toLocaleString()} / pack` : 'Coming Soon'}
                    </span>
                  </div>
                  {f.available ? (
                    <QuantityStepper
              ...
              {errors.flavors && <span className="font-sans text-xs text-red-400">{errors.flavors}</span>}
            </div>
            ...
            <div className="border border-brand-mid/30 p-3 md:p-5 flex flex-col gap-2">
              {FLAVORS.filter(f => (quantities[f.value] ?? 0) > 0).map(f => (
                <div key={f.value} className="flex justify-between font-sans text-xs text-brand-cream/50 tracking-wider uppercase">
                  <span>{quantities[f.value]} × {f.label}</span>
                  <span>₦{((quantities[f.value] ?? 0) * f.price).toLocaleString()}</span>
                </div>
              ))}
              {totalPacks === 0 && (
                <div className="font-sans text-xs text-brand-cream/30 tracking-wider uppercase">No packs selected</div>
              )}
              ...
              <div className="border-t border-brand-mid/30 pt-2 mt-1 flex justify-between font-sans text-sm text-brand-gold tracking-wider uppercase font-semibold">
                <span>Total</span>
                <span>{totalPacks > 0 && deliveryAvailable ? `₦${total.toLocaleString()}` : '—'}</span>
              </div>
            </div>

            {/* Submit */}
            {errors.submit && <p className="font-sans text-xs text-red-400 text-center">{errors.submit}</p>}
            {errors.state && <p className="font-sans text-xs text-brand-gold/70 text-center">{errors.state}</p>}

            <button
              type="submit"
              disabled={!deliveryAvailable || loading || totalPacks === 0}
